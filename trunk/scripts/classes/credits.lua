...
function Credits:calcInitPosY()
	if self.direction == -1 then
		self.posY = ((#self.Items * 14) + ((#self.Items - 1) * self.lineSpacing)) * -1
	else
		self.posY = 272
	end
end
...
function Credits:drawFrame()
	for a = 1, #self.Items, 1 do
		...
		screen.print(posX, self.posY + (a * 14) + (a-1) * self.lineSpacing, self.Items[a])
	end
end
...
function Credits:doneScrolling()
	x = false
	if self.direction == 1 then
		if self.posY < (-1 * ((#self.Items * 14) + ((#self.Items - 1) * self.lineSpacing))) then
			x = true
		end
	end
	if self.direction == -1 then
		if self.posY < (480 + (#self.Items * 14) + ((#self.Items - 1) * self.lineSpacing)) then
			x = true
		end
	end
	return x
